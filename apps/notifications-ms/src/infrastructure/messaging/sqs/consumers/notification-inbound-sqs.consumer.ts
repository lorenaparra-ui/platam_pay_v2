...
import { IngestNotificationSqsMessageUseCase } from '@messaging/application/use-cases/ingest-notification-sqs-message.use-case';
import {
  BaseConsumer,
  QUEUES_CONFIG,
  SQS_CLIENT,
...
/**
 * Consume jobs de NOTIFICATIONS_SQS_INBOUND_QUEUE_URL (email / SMS / WhatsApp).
 */
@Injectable()
export class NotificationInboundSqsConsumer
  extends BaseConsumer
  implements OnModuleInit, OnModuleDestroy
{
  private readonly nest_logger = new Logger(NotificationInboundSqsConsumer.name);

  constructor(
    @Inject(SQS_CLIENT) sqs_client: SQSClient,
    @Inject(QUEUES_CONFIG) private readonly queues_config: SqsQueuesUrlsConfig,
    private readonly config_service: ConfigService,
    private readonly ingest: IngestNotificationSqsMessageUseCase,
  ) {
    super(sqs_client, {
  ...
  protected async handle(message: SqsReceivedMessage): Promise<boolean> {
    ...
    this.nest_logger.log(
      `[Notify][step=consumer_handle][messageId=${message.message_id ?? 'n/a'}]`,
    );
