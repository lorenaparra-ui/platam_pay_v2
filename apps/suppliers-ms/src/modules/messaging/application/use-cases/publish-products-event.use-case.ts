...
import {
  ...
  type ProductsOutboundQueueUrlPort,
} from '@messaging/domain/ports/products-outbound-queue-url.port';
...
@Injectable()
export class PublishProductsEventUseCase {
  constructor(
    @Inject(OUTBOUND_MESSAGE_PUBLISHER_PORT)
    private readonly message_publisher: OutboundMessagePublisherPort,
    @Inject(PRODUCTS_OUTBOUND_QUEUE_URL_PORT)
    private readonly outbound_queue_url: ProductsOutboundQueueUrlPort,
  ) {}
  ...
  async execute(raw: unknown): Promise<void> {
    ...
    if (errors.length > 0) {
      const message = errors
        ...
        .join('; ');
      throw new ValidationFailedError(`Evento products-ms inválido: ${message}`);
    }
