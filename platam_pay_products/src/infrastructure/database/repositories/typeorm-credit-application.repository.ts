...
@Injectable()
export class TypeOrmCreditApplicationRepository
  implements CreditApplicationRepositoryPort
{
  constructor(
    @InjectRepository(CreditApplicationEntity)
    private readonly repository: Repository<CreditApplicationEntity>,
  ) {}
  ...
  async findAll(): Promise<CreditApplication[]> {
    ...
    return entities.map((e) => CreditApplicationMapper.toDomain(e));
  }

  async findById(id: number): Promise<CreditApplication | null> {
    const entity = await this.repository.findOne({ where: { id } });
    return entity ? CreditApplicationMapper.toDomain(entity) : null;
  }
  ...
  async create(
    ...
    const entity = CreditApplicationMapper.toEntityFromCreateInput(input);
    ...
    return CreditApplicationMapper.toDomain(full);
  }
  ...
  async updateByExternalId(
    ...
    if (input.personId !== undefined) entity.personId = input.personId;
    ...
    return updated ? CreditApplicationMapper.toDomain(updated) : null;
  }
